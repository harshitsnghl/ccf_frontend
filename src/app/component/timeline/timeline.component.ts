...
export class TimelineComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (!this.authService.isUserLoggedIn()) {
      ...
      if (this.router.url !== '/') {
        this.targetTagName =
          this.activatedRoute.snapshot.paramMap.get('tagName');
    ...
    if (!this.fetchingResult) {
      ...
      this.subscriptions.push(
        this.timelineService
          .getTimelinePosts(currentPage, this.resultSize, this.allPost)
          .subscribe({
  ...
  loadTaggedPosts(tagName: string, currentPage: number): void {
    if (!this.fetchingResult) {
      ...
      this.subscriptions.push(
        this.postService
          .getPostsByTag(tagName, currentPage, this.resultSize, this.allPost)
          .subscribe({
            next: (postResponseList: PostResponse[]) => {
              if (postResponseList.length === 0 && currentPage === 1)
                this.noPost = true;
